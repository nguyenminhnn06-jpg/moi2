...
siteController.index = async (req, res) => {
    ...
    let cates = await general.getCates()  
    
    // ✅ LỌC BỎ 3 DANH MỤC
    const categoriesToHide = ['Đèn năng lượng mặt tr��i', 'Đèn điện', 'Quạt điện']
    cates = cates.filter(cate => !categoriesToHide.includes(cate.category_name))
