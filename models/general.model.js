...
general.getCates = async () => {
    // Lấy TẤT CẢ danh mục để hiển thị trong form thêm/sửa sản phẩm
    let getCate = `SELECT c.*, COUNT(p.product_id) as category_count 
               FROM categories c 
               LEFT JOIN products p ON c.category_id = p.category_id 
               WHERE c.category_is_display = 1 
               GROUP BY c.category_id 
               ORDER BY c.category_id ASC`

    return new Promise((resolve, reject) => {
        console.log('🔍 Running getCates query...')
        
        db.query(getCate, async (err, cates) => {
            console.log('📊 Query result:', { error: err, catesCount: cates?.length })
            
            if (err) {
                console.error('❌ Error in getCates:', err)
                return resolve([])
            }
            ...
            console.log('✅ getCates resolved with', cates.length, 'categories')
            resolve(cates)
        })
    })
}
