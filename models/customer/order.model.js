...
order.updatePaidOrder = async (order_id, callback) => {
    let updatePaidOrder = `UPDATE orders 
                            SET orders.order_is_paid = 1,
                                orders.order_paying_date = current_date(),
                                orders.order_status = 'Đang giao hàng'
                            WHERE orders.order_id = ${order_id}`
    db.query(updatePaidOrder, (err, result) => {
        if (err) {
            console.log(err);
            callback(1, 0);
        } else {
