...
auth.loginPost = async (req, callback) => {
    const user_phone = req.body.user_phone;   // ✅ sửa lại
    const user_password = req.body.user_password; // ✅ sửa lại

    db.query('SELECT * FROM users WHERE user_phone = ?', [user_phone], async (err, result) => {
        if (err) return callback(1, 0, 0, 0, 0);

        if (!result.length) return callback(0, 1, 0, 0, 0);

        if (!await bcrypt.compare(user_password, result[0].user_password)) {
            return callback(0, 0, 1, 0, 0);
        }

        const id = result[0].user_id;
        return callback(0, 0, 0, 1, id);
    });
}
