...
const phoneNumber = document.getElementById('user_phone');
...
.then(res => {
    // Nếu server trả về redirect (ví dụ res.redirect("/"))
    if (res.redirected) {
        window.location.href = res.url;
        return;
    }
    // Nếu không redirect thì đọc JSON như bình thường
    return res.json();
})
.then(back => {
    if (!back) return; // đã redirect rồi, không cần xử lý tiếp
    if (back.status == "error") {
        setError(phoneNumber, back.error);
    } else if (back.status == "error2") {
        setError(password, back.error);
    } else if (back.status == "success") {
        window.location.href = '/'; // fallback chuyển về trang chủ
    }
})
