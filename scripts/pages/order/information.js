            ...
            .then(data => {
                ...
                elementHidden.innerHTML = `
                    <div class="product__view order-product__col-big">
                        <img src="/imgs/product_image/${productAvtImg}" alt="${productName}">
                        <p>${productName}</p>
                    </div>
    ...
    fetch('/order/information', {
        ...
        .then(back => {
            if (back.status === 'error') {
                ...
                if (back.paying_method_id == 1) {
                    window.location.href = `http://localhost:3000/order/vnpay?order_id=${back.order_id}`
                } else {
